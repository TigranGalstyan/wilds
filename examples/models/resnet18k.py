## ResNet18 for CIFAR
## From: https://gitlab.com/harvard-machine-learning/double-descent/-/blob/master/models/resnet18k.py
...
class PreActResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, init_channels=64, d_out=None, num_input_channels=50):
        super(PreActResNet, self).__init__()
        self.in_planes = init_channels
        c = init_channels

        self.conv1 = nn.Conv2d(num_input_channels, c, kernel_size=3, stride=1, padding=1, bias=False)
...
def make_resnet18k(d_out, k=64, num_classes=10, num_input_channels=50) -> PreActResNet:
    ...
    return PreActResNet(PreActBlock, [2, 2, 2, 2], num_classes=num_classes, init_channels=k,
                        d_out=d_out, num_input_channels=num_input_channels)
